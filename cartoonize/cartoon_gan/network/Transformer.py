...
class InstanceNormalization(nn.Module):
    def __init__(self, dim, gpu, eps=1e-9):
        super(InstanceNormalization, self).__init__()
        self.gpu = gpu
        if self.gpu:
            self.scale = nn.Parameter(torch.cuda.FloatTensor(dim))
            self.shift = nn.Parameter(torch.cuda.FloatTensor(dim))
        else:
