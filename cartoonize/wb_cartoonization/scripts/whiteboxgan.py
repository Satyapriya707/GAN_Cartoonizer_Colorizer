...
def infer_fn(model: WhiteBoxGAN, args_list: List[str]):
  ...
  parse.add_argument('--image_path',
                     help='input path, can be image,video,directory',
                     type=str)
  ...
  def infer_one_image(image_path: Path, output_root: Path):
    im = imread(image_path.as_posix(), args.load_size)
  ...
  def infer_video(video_path: Path, output_root: Path, extension, batch_size=16):
    ...
    output_path = output_root / (image_path.stem + '_out' + f'{extension}')
  ...
  if image_path.is_file():
    ...
    # if image_path.suffix in ['.mp4', '.flv', '.gif']:
    #   infer_video(image_path, output_root, image_path.suffix, args.batch_size)
    # else:
    ...
    for c, p in enumerate(image_path.iterdir()):
      t0 = time.time()
      if os.path.isfile(p):
        infer_one_image(p, output_root)
        print(f"inference  for image {c+1}/{img_num} took {time.time() - t0} s")
        t0 = time.time()
      else:
        print(f"{c+1}/{img_num} is not an image")
